...
const processJob = async (job: Job): Promise<Buffer> => {
    try {
        console.log(
            `Worker ${pdfQueueName}: Начал создавать PDF #${job.data.invoice.id} с данными:`, job.data.invoice);
...
const setupWorkerEvents = () => {
    pdfWorker.on("completed", async (job, result: Buffer) => {
        console.log(`Worker '${pdfQueueName}': PDF #${job.data.invoice.id} успешно создан. Результат:`, result);
    ...
    pdfWorker.on("failed", (job: Job, error: Error) => {
        try {
            console.log(`Worker '${pdfQueueName}': Ошибка при создании PDF #${job?.data.invoice.id}`, error);
        } catch (error) {
            throw error;
        }
    });
